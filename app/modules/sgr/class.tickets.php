...
class _hOrders {
  ...
  function getOrders() { ?>
    ...
    $products = $GLOBALS['JBLDB'] -> query( "SELECT * FROM ". _DBPREFIX ."posts LEFT JOIN hproducts ON hproducts.id = ". _DBPREFIX ."posts.id WHERE ilk = 'product'" );
    ...
    if ( !empty( $products_array) ) { 
      ...
      <div class="mdl-cell mdl-cell--3-col mdl-card mdl-shadow--2dp mdl-color--<?php primaryColor(); ?>">
        <div class="mdl-card-media">
          <img src="<?php echo $products_array[$key]["avatar"]; ?>" width="100%" style="overflow: hidden;" >
        </div>
        <form enctype="multipart/form-data" method="post" action="./shop?view=list&buy=add&code=<?php echo $products_array[$key]["id"]; ?>">
          ...
          <a href="?fav=<?php echo $products_array[$key]["id"]; ?>" class="mdl-button mdl-js-ripple-effect mdl-button--icon"><i class="material-icons">favorite</i></a>
          </div>
        </form>
      </div><?php 
      }
  ...
  function getOrder( $code) { ?>
    ...
    $product = $GLOBALS['JBLDB'] -> query( "SELECT * FROM ". _DBPREFIX ."posts LEFT JOIN hproducts ON hproducts.id = ". _DBPREFIX ."posts.id WHERE ". _DBPREFIX ."posts.id='". $_GET["view"] ."'" );
    ...
    <div class="mdl-cell mdl-cell--8-col mdl-card mdl-shadow--2dp mdl-color--<?php primaryColor(); ?>">
      ...
      <form enctype="multipart/form-data" method="post" action="./shop?view=list&buy=add&code=<?php echo $products_array[$key]["id"]; ?>">
        ...
        <div class="mdl-card__menu">
        <a href="?fav=<?php echo $product_deets[0]["id"]; ?>" class="mdl-button mdl-js-ripple-effect mdl-button--icon"><i class="material-icons">favorite</i></a>
        </div>
      </form>
    </div<?php 
    }

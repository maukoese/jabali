<?php 
namespace Modules\Sgr;
use Jabali\Classes\Resources;

class Coaches extends Resources {

  function getCoaches() { ?>
    <title>All Coaches - <?php showOption( 'name' ); ?></title><?php 
    $coaches = $GLOBALS['JBLDB'] -> query( "SELECT * FROM ". _DBPREFIX ."resources LEFT JOIN hcoaches ON ". _DBPREFIX ."coaches.id = ". _DBPREFIX ."resources.id WHERE ilk = 'coach'" );
    ...
    if ( !empty( $coaches_array) ) { 
      ...
      <div class="mdl-cell mdl-cell--3-col mdl-card mdl-shadow--2dp mdl-color--<?php primaryColor(); ?>">
        <div class="mdl-card-media">
          <img src="<?php echo $coaches_array[$key]["avatar"]; ?>" width="100%" style="overflow: hidden;" >
        </div>
        <form enctype="multipart/form-data" method="post" action="./shop?view=list&buy=add&code=<?php echo $coaches_array[$key]["id"]; ?>">
          <div class="mdl-card__title mdl-card--expand">
              <div class="mdl-card__title-text">
                <?php echo $coaches_array[$key]["name"]; ?>
              </div>
          ...
          <a href="?fav=<?php echo $coaches_array[$key]["id"]; ?>" class="mdl-button mdl-js-ripple-effect mdl-button--icon"><i class="material-icons">favorite</i></a>
          </div>
        </form>
      </div><?php 
      }
  ...
  function getCoachesClass( $class ) { ?>
    <title><?php echo( ucwords( $class ) ); ?> Coaches - <?php showOption( 'name' ); ?></title><?php 
    $coaches = $GLOBALS['JBLDB'] -> query( "SELECT * FROM ". _DBPREFIX ."resources LEFT JOIN hcoaches ON hcoaches.id = hresources.id WHERE hresources.ilk = 'coach' AND hcoaches.ilk = '".$class."'" );
    if ( $coaches -> num_rows > 0) {
      while( $row = mysqli_fetch_assoc( $coaches) ) {
        $coaches_array[] = $row;
      }
  ...
  function getCoach( $code) {
    $coach = $GLOBALS['JBLDB'] -> query( "SELECT * FROM ". _DBPREFIX ."resources LEFT JOIN hcoaches ON hcoaches.id = hresources.id WHERE hresources.id='". $_GET["view"] ."'" );
    if ( $coach -> num_rows > 0) { 
      while( $coach_array = mysqli_fetch_assoc( $coach) ) {
        $coach_deets[] = $coach_array;?>
    <title><?php echo( $coach_deets[0]['name'] ); ?> - <?php showOption( 'name' ); ?></title><?php 
      }
    } ?>

    <div class="mdl-cell mdl-cell--8-col mdl-card mdl-shadow--2dp mdl-color--<?php primaryColor(); ?>">
      <form enctype="multipart/form-data" method="post" action="./shop?view=list&buy=add&code=<?php echo $coach_deets[0]["id"]; ?>">
        <div class="mdl-card__title mdl-card--expand">
            <div class="mdl-card__title-text">
              <?php echo $coach_deets[0]["name"]; ?>
            </div>
            <div class="mdl-layout-spacer"></div>
            <div class="mdl-card__subtitle-text">
            </div>

    ...
    <div class="mdl-cell mdl-cell--8-col mdl-card mdl-shadow--2dp mdl-color--<?php primaryColor(); ?>">
      ...
      <form enctype="multipart/form-data" method="post" action="./shop?view=list&buy=add&code=<?php echo $products_array[$key]["h_code"]; ?>">
        ...
        <a href="?x=banda&product=all&fav=<?php echo $product_deets[0]["h_code"]; ?>" class="mdl-button mdl-js-ripple-effect mdl-button--icon"><i class="material-icons">favorite</i></a><?php } else { ?>
          <a href="?x=banda&edit=<?php echo $product_deets[0]["h_code"]; ?>&key=product" class="mdl-button mdl-js-ripple-effect mdl-button--icon"><i class="material-icons">edit</i></a><?php
        } ?>
        </div>
      </form>
    </div<?php 

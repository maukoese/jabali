...
class _hArticles {
  ...
  function createArticle() {
    ...
    $createArticle = mysqli_query($GLOBALS['conn'], "INSERT INTO harticles (h_alias, h_author, h_avatar, h_category, h_center, h_code, h_created, h_custom, h_description, h_email, h_fav, h_key, h_level, h_link, h_location, h_notes, h_phone, h_reading, h_status, h_style, h_subtitle, h_tags, h_text, h_type) 
  ...
  function getArticles() {
      echo "<title>All Articles [ <?php getOption('name'); ?> ]</title>";
    $getArticles = mysqli_query($GLOBALS['conn'], "SELECT * FROM harticles WHERE (h_type = 'article' AND h_status = 'published') ORDER BY h_created DESC");
  ...
  function getArticleCode($code) {
    ...
    if($getArticleCode -> num_rows > 0) {
      while ($articleDetails = mysqli_fetch_assoc($getArticleCode)){
        echo '<title>'.$articleDetails['h_alias'].' [ JABALI Articles ]</title>';
    ...
    if($getArticleCode -> num_rows > 0) {
      while ($articleDetails = mysqli_fetch_assoc($getArticleCode)){
        ...
        echo '<title>'.$articleDetails['h_alias'].' Article Edit [ '.getOption('name').' ]</title>';
